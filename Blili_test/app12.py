...
@app.route('/')
def show_all():
    return render_template('Blili_test_templates/show12.html', contacts=ContactDAO.query.all())
    # 查询所有记录并显示
...
def do_add():
    ...
    if request.method == 'POST':
        if not form1.validate():
            ...
            return render_template("Blili_test_templates/add12.html", form=form1)
            # 继续保持为添加界面，且保留原输入
        else:
...
def do_update(id):
    ...
    if request.method == 'GET':
        ...
        return render_template('Blili_test_templates/update12.html', form=form1)
